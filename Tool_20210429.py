...
from sklearn.linear_model import SGDRegressor,BayesianRidge,LogisticRegression
            ...
            for t in tol:
                for c in C:
                    if(nums>=num):
                        ...
                        model = LogisticRegression(warm_start=True,alpha=a,normalize_y=n,random_state=random_state,n_jobs=-1)
                        for batch in self.batch_data:
                            model.partial_fit(batch[0],batch[1])
                        pred_test = model.predict(self.test_x)
                        pred_test = pred_test.reshape(-1,1)
