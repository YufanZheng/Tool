...
from sklearn.metrics import d2_tweedie_score,mean_tweedie_deviance,mean_pinball_loss,mean_absolute_error,mean_squared_error,median_absolute_error,r2_score,mean_squared_log_error,explained_variance_score,max_error,mean_poisson_deviance,mean_gamma_deviance
...
class model:
    ...
    def calculate(self,y_true, y_predict, n, p ,pf=False):
        ...
        try:
            ...
            pass
        try:
            me = max_error(y_true, y_predict)
        except:
            pass
        try:
            mpd = mean_poisson_deviance(y_true, y_predict)
        except:
            pass
        try:
            mgd = mean_gamma_deviance(y_true, y_predict)
        except:
            pass
        try:
            pl = mean_pinball_loss(y_true, y_predict,alpha=0.95)
        except:
            pass
        try:
            mtd_esd = mean_tweedie_deviance(y_true, y_predict,power=-1)
        except:
            pass
        try:
            mtd_pd = mean_tweedie_deviance(y_true, y_predict,power=1)
        except:
            pass
        try:
            mtd_cpd = mean_tweedie_deviance(y_true, y_predict,power=1.5)
        except:
            pass
        try:
            mtd_gd = mean_tweedie_deviance(y_true, y_predict,power=2)
        except:
            pass
        try:
            mtd_igd = mean_tweedie_deviance(y_true, y_predict,power=3)
        except:
            pass
        try:
            mtd_psd = mean_tweedie_deviance(y_true, y_predict,power=5)
        except:
            pass

        try:
            d2_esd = d2_tweedie_score(y_true, y_predict,power=-1)
        except:
            pass
        try:
            d2_pd = d2_tweedie_score(y_true, y_predict,power=1)
        except:
            pass
        try:
            d2_cpd = d2_tweedie_score(y_true, y_predict,power=1.5)
        except:
            pass
        try:
            d2_gd = d2_tweedie_score(y_true, y_predict,power=2)
        except:
            pass
        try:
            d2_igd = d2_tweedie_score(y_true, y_predict,power=3)
        except:
            pass
        try:
            d2_psd = d2_tweedie_score(y_true, y_predict,power=5)
        except:
            pass
        if(pf):
            try:
            ...
            try:
                ...
                pass
            try:
                print("MTD_IGD: ",mtd_igd)
            except:
                pass
            try:
                print("MTD_PSD: ",mtd_psd)
            except:
                pass
            ...
            try:
                ...
                pass
        return [mse,rmse,mae,r2,mad,mape,r2_adjusted,rmsle,evs,me,mpd,mgd,pl,mtd_esd,mtd_pd,mtd_cpd,mtd_gd,mtd_igd,mtd_psd,d2_esd,d2_pd,d2_cpd,d2_gd,d2_igd,d2_psd]
    ...
    def Ada(self,name):
        ...
        if(os.path.exists(path_a)):
            ...
            nums = 0
            col_a = ['num','mse','rmse','mae','r2','mad','mape','r2_adjusted','rmsle','evs','me','mpd','mgd','pl','mtd_esd','mtd_pd','mtd_cpd','mtd_gd','mtd_igd','mtd_psd','d2_esd','d2_pd','d2_cpd','d2_gd','d2_igd','d2_psd']
            ...
            for l in learning_rate:
                for lo in loss:
                    if(nums>=num):
                        ...
                        all_m = self.calculate(self.test_y,pred_test,self.test_sample_size,self.feature_size)
                        all_m.insert(0,num)
                        # all_m = [num,mse,rmse,mae,r2,mad,mape,r2_adjusted,rmsle,evs,me,mpd,mgd,pl,mtd_esd,mtd_pd,mtd_cpd,mtd_gd,mtd_igd,mtd_psd,d2_esd,d2_pd,d2_cpd,d2_gd,d2_igd,d2_psd]
                        all_p = [num,n,l,lo]
                        self.write_csv_result(path_a,path_p,all_m,all_p)
                        if(all_m[2] < best_result):
                            joblib.dump(model,model_path)
                            with open(path_b,"a",encoding="utf-8",newline="")as f:
                                f = csv.writer(f)
                                f.writerow(all_m)
                            best_result = all_m[2]
                            print("current best result, mse:{},r2:{},mape:{}".format(all_m[1],all_m[4],all_m[6]))
                        print("end....",num)
                        num = num+1
                        print("--------------------------------")
    ...
    def Lgb(self,name):
        ...
        reg_alpha = [0,1,2]
        reg_lambda  = [0,1,2]
        all_nb = len(boosting_type)*len(n_estimators)*len(learning_rate)*len(reg_alpha)*len(reg_lambda)*len(colsample_bytree)
        num = 1
        # 用于重启训练模型，提高效率，不重复跑相同的实验
        if(os.path.exists(path_a)):
            data = pd.read_csv(path_a)
            ...
            nums = 0
            col_a = ['num','mse','rmse','mae','r2','mad','mape','r2_adjusted','rmsle','evs','me','mpd','mgd','pl','mtd_esd','mtd_pd','mtd_cpd','mtd_gd','mtd_igd','mtd_psd','d2_esd','d2_pd','d2_cpd','d2_gd','d2_igd','d2_psd']
            col_p = ['num','boosting_type','n_estimators','learning_rate','colsample_bytree','reg_alpha','reg_lambda']
            self.write_csv_result(path_a,path_p,col_a,col_p)
        # 用于保存最好的模型
        if(os.path.exists(path_b)):
            best_results = pd.read_csv(path_b)
                        ...
                        for r1 in reg_alpha:
                            for reg_l in reg_lambda:
                                if(nums>=num):
                                    ...
                                    model = lgb.LGBMRegressor(reg_alpha=r1,reg_lambda=reg_l,colsample_bytree=c,learning_rate=l,n_estimators=n,boosting_type=b,objective='regression',random_state=random_state)
                                    model.fit(self.train_x,self.train_y)
                                    pred_test = model.predict(self.test_x)
                                    pred_test = pred_test.reshape(-1,1)
                                    all_m = self.calculate(self.test_y,pred_test,self.test_sample_size,self.feature_size)
            ...
            for t in tol:
                for c in C:
                    if(nums>=num):
                        ...
                        model = LogisticRegression(penalty=p,tol=t,C=c,warm_start=True,random_state=random_state,n_jobs=-1,solver = solver)
                        model.fit(self.train_x,self.train_y)
                        pred_test = model.predict(self.test_x)
                        pred_test = pred_test.reshape(-1,1)
                        all_m = self.calculate(self.test_y,pred_test,self.test_sample_size,self.feature_size)
