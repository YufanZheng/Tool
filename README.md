...
Welcome to use and correct！！！

## Email：zhjpre@gmail.com  
Email: zhjpre@gmail.com

## 此工具只用于提高工作效率，请在调用工具包的同时，注重个人理论知识的积累，切勿成为一个调包侠！！！  
This tool is only for improving work efficiency. Please focus on accumulating your theoretical knowledge while using the toolkit, and avoid becoming a tool swapper!!!

## 每个模型的参数范围需自行针对每个任务进行合理的设定。  
The parameter range of each model needs to be reasonably set for each task.
...
| Name（名称）      | Description（描述） | Type（类型）     |   Other（其他）  |
| :----:        |    :----:   |    :----:   |    :----:   |
| file_name      |  查找指定文件夹名下的所有文件的名称 (Find all file names in a specified folder) |  function（函数）  |    |
| mkdir   |  创建文件夹 (Create a folder) | function（函数）  |    |
| create_time_seq   |  创建时间序列数据集 (Create a time series dataset)  |  function（函数）  |     |
| BLSregressor   |  封装好的 Bord Learning System 的回归模型类 (The class of the regression model for the Bord Learning System)  | class（类）  |     |
| BLSclassification   |  封装好的 Bord Learning System 的分类模型类 (The class of the classification model for the Bord Learning System)  | class（类）  |     |
